...
type GlobalWindow = Window & {
  [ACTIVE_BASE_GLOBAL]?: string
  __VP_SITE_DATA__?: VitePressSiteData
}
...
function readSiteBase(globalWindow: GlobalWindow | undefined) {
...
function readDeclaredBase(globalWindow?: GlobalWindow) {
  return readMetaBase() ?? readSiteBase(globalWindow) ?? readEnvBase() ?? '/'
}

function computeActiveBase(globalWindow: GlobalWindow, declaredBase: string) {
...
export function withActiveBase(path: string, baseOverride?: string) {
