...
1. **职责过载的串行流程**：核心 orchestrator 现已引入 `PagegenPluginRegistry` 与 `PagegenScheduler`，可通过 `--max-parallel`/`--no-parallel` 调整并行度，但仍需补充插件 SDK、并行压测与回退文档，确保扩展者理解生命周期钩子。【F:scripts/pagegen.mjs†L104-L187】
...
> 顺序建议：优先产出插件 SDK 指南与示例，随后执行并行压测/回退验证；待基线稳定后，再并行推进 `build.ai` 可视化与模型 Schema/运维手册工作。
