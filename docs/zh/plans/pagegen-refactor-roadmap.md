...
- 变更感知重建：结合 Git diff 与缓存快照，仅重建受影响的语言/聚合目录，并提供 `--full-build` 等回退 flag，确保异常时可快速切换回全量。
- 指标时间序列：扩展 telemetry/metrics 管线，写入历史快照并在“观测指标”页展示趋势或阈值告警，方便巡检与回归分析。
- AI 产出评测：为 `scripts/ai/*` 接入离线评测夹具、基线样本与阈值判定，在真实模型接入前确保可回退至占位实现。
...
- **反馈同步机制**：本节会随巡检推进持续更新，关键结论亦会同步至 `AGENTS.md` 第 10 节，确保多代理协作信息一致。当前正在为阶段 6 的变更感知、指标时间序列与 AI 守门准备实现计划。
