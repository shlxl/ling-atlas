...
- 下一阶段重点：
  ...
  3. 📊 AI 遥测可视化：在 telemetry 页面与 GitHub Step Summary 呈现 `build.ai` 摘要，记录最近一次 `ai.*` 事件的耗时、适配器与处理条目。
  4. 🧾 模型配置 Schema 化：设计 `schema/models.json` + 校验脚本，完善 `ai:prepare`/`ai:smoke` 的运维文档与回滚流程。
  > 推荐节奏：先完成插件 SDK（1）以稳定扩展契约，再执行并行压测（2）；在并行行为确认后，可并行推进遥测可视化（3）与模型 Schema 化（4）。
...
- AI 构建脚本输出 `ai.embed.*`/`ai.summary.*`/`ai.qa.*` 遥测事件，`tests/ai/telemetry.integration.test.mjs` 覆盖事件写入与 `build.ai` 汇总，`scripts/telemetry-merge.mjs` 会清理消费后的事件文件。
- Pagegen orchestrator 引入 `PagegenPluginRegistry` 与 `PagegenScheduler`，支持 `--max-parallel`/`--no-parallel` 并行调度，保持回退兼容；并行管线已在 `npm run test:pagegen` 覆盖。
- 新增 `npm run ai:prepare`、`npm run ai:smoke` 及 CI 可选冒烟步骤，`tests/ai/lifecycle.test.mjs` 验证模型复制、校验与失败告警，`.gitignore` 统一忽略 `data/models/` 缓存。
