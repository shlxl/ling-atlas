...
await (async () => {
  ...
  async function runLocalePipeline(lang) {
    ...
    const rssLimitedSuffix = localeFeed.rssLimited ? ' (limited)' : ''
    logInfo(
      `[pagegen] feeds:${lang.manifestLocale} rss=${localeFeed.rssCount}${rssLimitedSuffix} sitemap=${localeFeed.sitemapCount}`
    )
    ...
    async () => {
      for (const { lang, payload } of navPayloads) {
        ...
        if (totalEntries === 0) {
          logStageWarning(
            'nav-manifest',
            { locale: lang.manifestLocale, target: lang.navManifestPath },
            'manifest empty; please verify nav configuration and generated aggregates'
          )
        } else {
          logInfo(summaryText)
        }
