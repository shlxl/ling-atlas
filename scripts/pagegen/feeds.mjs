...
const FEEDS_TEMPLATE_PATH = path.join(ROOT_DIR, 'schema', 'feeds.templates.json')
...
export async function generateRss(
  lang,
  items = [],
  { publicDir, siteOrigin = '', preferredLocale, writer, dryRun = false, templates } = {}
) {
...
async function fallbackGenerateRss(lang, items, { publicDir, siteOrigin, preferredLocale, writer, dryRun }) {
  ...
  if (!dryRun) {
    await scheduleWrite('rss', lang, publicDir, lang?.rssFile, content, { writer, dryRun })
  }

  return { count: sliceCount, limited }
}

async function fallbackGenerateSitemap(lang, items, { publicDir, siteOrigin, writer, dryRun }) {
  const totalItems = Array.isArray(items) ? items.length : 0
  if (!totalItems) return { count: 0 }
  ...
  if (!dryRun) {
    await scheduleWrite('sitemap', lang, publicDir, lang?.sitemapFile, xml, { writer, dryRun })
  }
