...
if (startIndex === -1 || endIndex === -1) {
  const missing = htmlEntries.filter(entry => !swSource.includes(`"${entry}"`))
  if (missing.length === 0) {
    console.info('[pwa] service worker already includes HTML entries; skipping patch.')
    process.exit(0)
  }

  console.warn('[pwa] unable to locate precache manifest in service worker; HTML entries missing:', missing.join(', '))
  process.exit(0)
}
...
for (const relativePath of htmlEntries) {
  if (swSource.includes(`url:"${relativePath}"`) || swSource.includes(`"${relativePath}"`)) continue
