...
## 3. URL 与目录结构
- 每篇文章的物理位置固定：`docs/<locale>/content/<slug-or-custom>/index.md`（例如默认中文位于 `docs/zh/content/**`）。
- 页面访问路径（VitePress）默认：`/<locale>/content/<slug-or-custom>/`（例如默认中文位于 `/zh/content/**`，项目站点会自动加 `base` 前缀）。
